...
*  Příjem audia & volání Whisperu běží paralelně → fronta nikdy nepřeteče  
*  `audio_receiver_size = 1024` (≈ 20 s audia)  
...
AUDIO_BATCH_SECONDS   = 5      # délka WAV dávky do Whisperu
RECEIVER_SIZE_FRAMES  = 1024   # velikost fronty streamlit-webrtc
...
def render_flipchart() -> None:
    ...
    if not st.session_state.flip_points:
        st.info("Čekám na shrnutí…")
        return
    bullet_list = "".join(
        f"<li style='animation-delay:{i*0.1}s'>{format_point(p)}</li>"
        for i, p in enumerate(st.session_state.flip_points)
    )
    st.markdown(f"<ul class='flipchart'>{bullet_list}</ul>", unsafe_allow_html=True)
...
with tab_ctrl:
    ...
    async def reader(ctx) -> None:
        sr = 48000
        tgt = AUDIO_BATCH_SECONDS * sr * 2  # bytes pro 5 s
