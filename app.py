...
--------------------------------------
• Mikrofon / upload ↦ Whisper (transkript)  
• Transkript POST na Make → Make vrátí JSON pole nových bodů  
• Flipchart: NADPISY tučně + pododrážky se symbolem •  
• WebRTC = SENDONLY  (žádná ozvěna)
"""

from __future__ import annotations
import asyncio, contextlib, io, logging, threading, wave
...
def call_make(transcript: str, existing: list[str]) -> list[str]:
    try:
        ...
        if not isinstance(data, list):
            logging.error("Make response not list: %s", data)
...
def format_point(raw: str) -> str:
    ...
    Převod:
        ...
        - detail 1
        - detail 2
    → <strong>NADPIS</strong><ul><li>detail 1</li>…</ul>
    """
    ...
    if not details:
        return heading_html
    items = "".join(f"<li>{d.lstrip('-• ').strip()}</li>" for d in details)
    ...
    async def audio_pipeline(ctx, stop_event):
        ...
        while not stop_event.is_set():
            ...
            if len(st.session_state.transcript_buffer.split()) >= 325:
                ...
                st.session_state.flip_points.extend([p for p in new_pts if p not in st.session_state.flip_points])
                st.session_state.transcript_buffer = ""
            await asyncio.sleep(0.05)
        set_status("⏹️ Audio pipeline ukončena")

    t = threading.Thread(target=lambda c=webrtc_ctx,e=stop_evt: asyncio.run(audio_pipeline(c,e)),
                         daemon=True)
