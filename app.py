...
🖥️ UI a funkce
──────────────
✓ WebRTC SENDRECV  +  sendback_audio=False   (neslyším sám sebe)  
✓ TURN tcp/443 (openrelay.metered.ca)  +  iceTransportPolicy="relay"  
✓ Audio batch = 60 s  (≈ 5 ,8 MB WAV) – Make má ~30 s na zpracování  
✓ Viditelný stav v UI + v sidebaru  
✓ Nadpisy tučně, podbody se symbolem •; funguje pro oba formáty odrážek  
✓ run_async_forever()  → žádný „no running event loop“ v logu
"""
...
AUDIO_BATCH_SECONDS = 60          # 1min dávka (≈5,8 MB WAV)
...
def call_make(transcript: str, existing: list[str]) -> list[str]:
    ...
    try:
        ...
        set_status("⚠️ Chyba Make"); st.error(exc); return []
...
def format_point(raw: str) -> str:
    ...
    if not details: return head_html
...
with tabs[0]:
    ...
    # Viditelný stav přímo pod mikrofonem
    ...
    async def audio_pipeline(ctx, stop_event):
        SR = 48000; target = AUDIO_BATCH_SECONDS*SR*2  # bytes v dávce
        while not stop_event.is_set():
            if not ctx.audio_receiver:
                set_status("🟡 Čekám na mikrofon…"); await asyncio.sleep(0.1); continue

            set_status(f"🔴 Nahrávám … ({AUDIO_BATCH_SECONDS}s dávka)")
