...
from streamlit_extras.app_autorefresh import st_autorefresh  # ← diagnostika
...
with tab1:
    ...
    if up and not st.session_state.upload_processed:
        with status_ctx("🟣 Whisper (upload)…"): t=whisper_safe(up,"upload")
        if t:
            with status_ctx("📤 Odesílám do Make…","🟢 Čekám Make"): new=call_make(t,[])
    ...
    with st.expander("📡 WebRTC diagnostika", expanded=False):
        # auto-refresh jen uvnitř expanderu
        ...
        if pc:
            ...
            st.info("PeerConnection ještě nevznikl.")
    ...
    th=threading.Thread(
        target=lambda c=webrtc_ctx,e=stop_evt: run_async_forever(pipeline(c,e)),daemon=True)
