...
for k, v in {
    ...
    "runner_thread": None,
    "last_frame_time": 0.0,          # ⬅️ poslední přijatý rámec
}.items():
    s.setdefault(k, v)
...
def render_flip() -> None:
    ...
    html = "<ul class='flipchart'>" + "".join(
        f"<li style='animation-delay:{i*.1}s'>{fmt(p)}</li>"
...
with tab_ctrl:
    ...
    # Ukonči staré vlákno při rerunu
    ...
    async def pipeline():  # dvě korutiny paralelně
    ...
    # živý indikátor mikrofonu
    if time.time() - s.get("last_frame_time", 0) < 2:
...
with tab_flip:
    st.components.v1.html(
        "<script>document.body.classList.add('fullscreen');</script>",
        height=0,
    )
    render_flip()
